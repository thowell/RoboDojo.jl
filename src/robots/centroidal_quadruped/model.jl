...
function mass_matrix(model::CentroidalQuadruped, q)
    cat(
...
function dynamics_bias(model::CentroidalQuadruped, q, q̇)
    [
...
function signed_distance(model::CentroidalQuadruped, q)
...
function contact_jacobian(model::CentroidalQuadruped, q) 
    z3 = zeros(3, 3)
...
centroidal_quadruped_contact_kinematics = [
    q -> q[6  .+ (1:3)],
    q -> q[9  .+ (1:3)],
    q -> q[12 .+ (1:3)],
    q -> q[15 .+ (1:3)],
...
centroidal_quadruped_contact_kinematics_jacobians = [
...
name(::CentroidalQuadruped) = :centroidal_quadruped
floating_base_dim(::CentroidalQuadruped) = 6
