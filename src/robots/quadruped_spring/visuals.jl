function build_robot!(vis::Visualizer, model::QuadrupedSpring;
	...
	torso = MeshCat.GeometryBasics.Cylinder(MeshCat.Point3f0(0.0, 0.0, -model.l_torso2),
		MeshCat.Point3f0(0.0, 0.0, model.l_torso1),
		convert(Float32, r_body))
	setobject!(vis[:robot]["torso"], torso, body_mat)

	thigh_1 = MeshCat.GeometryBasics.Cylinder(MeshCat.Point3f0(0),
		MeshCat.Point3f0(0.0, 0.0, model.l_thigh1),
		convert(Float32, r_thigh))
	setobject!(vis[:robot]["thigh1"], thigh_1, body_mat)

	calf_1 = MeshCat.GeometryBasics.Cylinder(MeshCat.Point3f0(0),
		MeshCat.Point3f0(0.0, 0.0, model.l_calf1),
		convert(Float32, r_calf))
	setobject!(vis[:robot]["calf1"], calf_1, body_mat)

	thigh_2 = MeshCat.GeometryBasics.Cylinder(MeshCat.Point3f0(0),
		MeshCat.Point3f0(0.0, 0.0, model.l_thigh2),
		convert(Float32, r_thigh))
	setobject!(vis[:robot]["thigh2"], thigh_2, body_mat)

	calf_2 = MeshCat.GeometryBasics.Cylinder(MeshCat.Point3f0(0),
		MeshCat.Point3f0(0.0, 0.0, model.l_calf2),
		convert(Float32, r_calf))
	setobject!(vis[:robot]["calf2"], calf_2, body_mat)

	thigh_3 = MeshCat.GeometryBasics.Cylinder(MeshCat.Point3f0(0),
		MeshCat.Point3f0(0.0, 0.0, model.l_thigh3),
		convert(Float32, r_thigh))
	setobject!(vis[:robot]["thigh3"], thigh_3, body_mat)

	calf_3 = MeshCat.GeometryBasics.Cylinder(MeshCat.Point3f0(0),
		MeshCat.Point3f0(0.0, 0.0, model.l_calf3),
		convert(Float32, r_calf))
	setobject!(vis[:robot]["calf3"], calf_3, body_mat)

	thigh_4 = MeshCat.GeometryBasics.Cylinder(MeshCat.Point3f0(0),
		MeshCat.Point3f0(0.0, 0.0, model.l_thigh4),
		convert(Float32, r_thigh))
	setobject!(vis[:robot]["thigh4"], thigh_4, body_mat)

	calf_4 = MeshCat.GeometryBasics.Cylinder(MeshCat.Point3f0(0),
		MeshCat.Point3f0(0.0, 0.0, model.l_calf4),
		convert(Float32, r_calf))
	setobject!(vis[:robot]["calf4"], calf_4, body_mat)

	hip1 = setobject!(vis[:robot]["hip1"],
		MeshCat.GeometryBasics.Sphere(MeshCat.Point3f0(0), convert(Float32, r_hip)), body_mat)
	hip2 = setobject!(vis[:robot]["hip2"],
		MeshCat.GeometryBasics.Sphere(MeshCat.Point3f0(0), convert(Float32, r_hip)), body_mat)
	knee1 = setobject!(vis[:robot]["knee1"],
		MeshCat.GeometryBasics.Sphere(MeshCat.Point3f0(0), r_knee), body_mat)
	...
	feet1 = setobject!(vis[:robot]["feet1"],
		MeshCat.GeometryBasics.Sphere(MeshCat.Point3f0(0), r_foot), contact_mat)
...
function set_robot!(vis::Visualizer, model::QuadrupedSpring, q::AbstractVector;
...
function visualize!(vis, model::QuadrupedSpring, q;
	Δt=0.1,
	r_body=0.02, r_thigh=0.0175, r_calf=0.0125, r_hip=0.02, r_knee=0.02, r_foot=0.02,
	color_opacity=1.0)
